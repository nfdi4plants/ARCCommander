...
[<Tests>]
let testDataModelFunctions = 
    testList "DataModelFunctions" [
        testCase "GetKeyValues" (fun () -> 
            let testAssay = 
                [|("Measurement Type", "LookAtIt");
            ...
            let assayOfInterest = Assay(fileName="myAssay",measurementType="LookAtIt")

            Expect.sequenceEqual               
                (getKeyValues assayOfInterest)
                testAssay
                "GetKeyValues returned an unexpected array"
        )
        testCase "GetIdentificationKeyValues" (fun () -> 
            let testPerson = Person(firstName="Max",midInitials="P",lastName="Mustermann",phone="987654321",roles="BestGuy")
            let personOfInterest = Person(firstName="Max",midInitials="P",lastName="Mustermann",phone="0123456789",roles="Scientist,Engineer,GeneralExpert")
